angular.module('ui.bootstrap.datepicker', ['ui.bootstrap.dateparser', 'ui.bootstrap.isClass'])
  ...
  .controller('UibHmstimepickerController', ['$scope', '$element', 'dateFilter', '$log', '$filter', function (scope, $element, dateFilter, log, filter) {
    ...
    this._refreshView = function () {
      ...
      scope.$watch('time', function () {
        scope.selectedDt.setHours(scope.time.hour);
        scope.selectedDt.setMinutes(scope.time.minute);
        scope.selectedDt.setSeconds(scope.time.second);
      }, true)
    };

    function minus(hms) {
      hmsPlusOrMinus(hms, 'minus');
    }

    function plus(hms) {
      hmsPlusOrMinus(hms, 'plus');
    }

    function hmsPlusOrMinus(hms, op) {
      switch (hms) {
        case 'h':
          if (op == 'plus') {
            scope.time.hour++;
            if (scope.time.hour > 23) {
              scope.time.hour = 23;
            }
          } else if (op == 'minus') {
            scope.time.hour--;
            if (scope.time.hour < 0) {
              scope.time.hour = 0;
            }
          }
          break;
  ...
  .directive('uibHmstimepicker', function () {
    return {
      templateUrl: function (element, attrs) {
        return attrs.templateUrl || 'uib/template/datepicker/time.html';
      },
      require: ['^uibDatepicker', 'uibHmstimepicker'],
      restrict: 'A',
      controller: 'UibHmstimepickerController',
      link: function (scope, element, attrs, ctrls) {
        var datepickerCtrl = ctrls[0],
          hmstimepickerCtrl = ctrls[1];

        hmstimepickerCtrl.init(datepickerCtrl);
      }
    }
  })

  .directive('uibHourValid', function () {
    return {
      require: 'ngModel',
